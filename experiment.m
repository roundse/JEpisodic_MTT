...
EXT_CONNECT = .07;                   % Chance of connection = 20%
INT_CONNECT = .2;
...
w_food_to_pfc = .35 .* ones(FOOD_CELLS, PFC_SIZE);
w_pfc_to_food = w_food_to_pfc';
w_place_to_pfc = .35 .* ones(PLACE_CELLS, PFC_SIZE);
...
w_food_to_hpc = 0.42 .* (rand(FOOD_CELLS, HPC_SIZE) < EXT_CONNECT);
w_hpc_to_food = - w_food_to_hpc';
w_place_to_hpc = 0.42 .* (rand(PLACE_CELLS, HPC_SIZE) < EXT_CONNECT);
