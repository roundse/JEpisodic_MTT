...
w_food_to_pfc = .56 .* ones(FOOD_CELLS, PFC_SIZE);
w_pfc_to_food = w_food_to_pfc';
w_place_to_pfc = .56 .* ones(PLACE_CELLS, PFC_SIZE);
...
w_food_to_hpc = 0.12 .* (rand(FOOD_CELLS, HPC_SIZE) < EXT_CONNECT);
w_hpc_to_food = - w_food_to_hpc';
w_place_to_hpc = 0.12 .* (rand(PLACE_CELLS, HPC_SIZE) < EXT_CONNECT);
